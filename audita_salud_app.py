...
def find_birth_date(page_texts):
    ...
    for i, page in enumerate(page_texts):
        st.write(f"Depuración - Página {i+1}: Buscando 'Fecha Nacimiento' o 'Nacimiento'")
        ...
        if match_label:
            ...
            if date_match:
                ...
                st.write(f"Depuración - Fecha encontrada: {date_match.group(1)} -> {parsed_date}")
                return parsed_date
...
def find_patient_name(page_texts):
    ...
    for i, page in enumerate(page_texts[:2]):
        st.write(f"Depuración - Página {i+1}: Buscando 'Nombre Paciente'")
...
def find_identification(page_texts):
    ...
    for i, page in enumerate(page_texts[:2]):
        st.write(f"Depuración - Página {i+1}: Buscando 'Identificación' o 'Documento'")
        match = re.search(r"(?:Identificación|Documento)[\s:]*(\d+)", page, re.IGNORECASE)
        if match:
            st.write(f"Depuración - Identificación encontrada: {match.group(1)}")
...
def extract_text_pages(pdf_file):
    ...
    try:
        st.write(f"Depuración - Iniciando procesamiento de {pdf_file.name}, tamaño: {pdf_file.size} bytes")
        ...
        st.write(f"Depuración - Extracción completada: {len(pages)} páginas")
...
def extract_attentions(page_texts):
    ...
    for i, page_text in enumerate(page_texts):
        st.write(f"Depuración - Procesando página {i+1} para atenciones")
    ...
    if current_attention:
        attentions.append(current_attention)
    st.write(f"Depuración - Atenciones detectadas: {len(attentions)}")
    return attentions
...
with st.sidebar:
    ...
    st.markdown("---")
    st.success("Prototipo v9.2 - Depuración Avanzada.")
