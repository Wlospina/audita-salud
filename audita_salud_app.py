...
# --- CORRECCIÓN DEL BUG: Diccionario con claves numéricas (int) para los meses ---
MESES_MAP_FROM_NUM = {
...
def format_date_spanish(date_obj):
    """Función corregida que usa el diccionario con claves numéricas."""
    if not isinstance(date_obj, datetime): return "Fecha inválida"
    return f"{date_obj.day} de {MESES_MAP_FROM_NUM[date_obj.month]} de {date_obj.year}"

def parse_date_flexible(date_str):
    date_str_normalized = date_str.lower()
    for name, num in MESES_MAP_TO_NUM.items():
...
def process_pdf_data(_file_bytes):
    try:
        ...
        if doc.is_encrypted: return {"error": "El PDF está protegido."}
        
        page_texts = [page.get_text() for page in doc]
        doc.close()
        
        if not any(page.strip() for page in page_texts): return {"error": "El PDF no contiene texto."}
        
        patient_name, birth_date, identification = find_demographic_data(page_texts)

        # --- LÓGICA DE EXTRACCIÓN DE ATENCIONES REFINADA ---
        attentions = []
        last_date = None
        for page_text in page_texts:
            ...
            if is_new_attention and date_match:
                ...
                # Solo crea una nueva atención si la fecha es nueva
                if current_date != last_date:
                    attentions.append({"fecha_atencion": current_date, "contenido": page_text})
                    last_date = current_date
                # Si la fecha es la misma, es continuación de la anterior.
                elif attentions:
                    attentions[-1]["contenido"] += "\n" + page_text
...
with st.sidebar:
    ...
    st.markdown("---")
    st.success("Prototipo v11.0 - Estable.")
